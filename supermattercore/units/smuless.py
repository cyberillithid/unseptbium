"""An attempt to rewrite model to be astropy-less to check whether perofrmance would be as abysmal"""
...
ENG_CANISTER_PRESSURE = 90 * 101325 # Pa  atm
...
IDEAL_GAS_ENTROPY_CONSTANT = 1164 #* u.K*u.mol/u.l / (u.J*u.kg/u.mol**2)**(2/3)

CELL_VOLUME = 2.500 #* u.m**3
...
GasO2 = GasSpecies('O2',  0.032, is_oxy=True)
GasN2 = GasSpecies('N2',  0.028)
GasHe = GasSpecies('He',  0.004, cv= 80)
GasPh = GasSpecies('Ex*', 0.405, cv=200, is_fuel=True)
...
class HeatExchanger:
    ...
    def cool(self, gas: GasMixture):
        ...
        SmLogger.info(f'last {(eta*dQ)} J; Per {self.n} pipes {t0:.2f} -> {gas.T:.2f} ({t0-gas.T:.4g} loss)')
        return gas
...
class Pump:
    name: str
    V_in :   float = 0.2  # Quantity[u.L]= 200*u.L
...
class Turbine:
    ...
    def take_air(self) -> tuple[GasMixture|None, float]: #  Quantity[u.J]
        ...
        SmLogger.info(f'{self.name}: inP = {src.p/1e3:.4g}, outP={sink.p/1e3:.4g}')
        gret = src.sub(nu_use)
        SmLogger.info(f'{self.name}: inP[red] = {src.p/1e3:.4g}')
